  ...
  Future<void> _save() async {
    if (_selectedMealTime.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please select a reading type'), backgroundColor: Colors.red),
      );
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Reading updated'), backgroundColor: Colors.green),
        );
      ...
      child: Column(
        children: [
          Text(
            ...
            style: TextStyle(fontSize: 42, fontWeight: FontWeight.bold, color: _colorForLevel(_glucoseLevel)),
          ),
          const Text('mg/dL', style: TextStyle(fontSize: 16, color: Colors.grey)),
          Slider(
            value: _glucoseLevel,
            min: 40,
            max: 400,
